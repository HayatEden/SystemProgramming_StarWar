...
public abstract class MicroService implements Runnable {
    ...
    private ConcurrentHashMap<Class,Callback> whichCallBack; // defines for each event/broadcast, the matching callback
    protected boolean landoFinished;
    private String name;
    ...
    public MicroService(String name) {
        whichCallBack = new ConcurrentHashMap<Class,Callback>();
        landoFinished = false;
        this.name = name;
    ...
    protected final <T, E extends Event<T>> void subscribeEvent(Class<E> type, Callback<E> callback) {
        ...
        whichCallBack.put(type,callback);
    }
    ...
    protected final void terminate() {
        landoFinished = true; // makes the while loop in run to stop and therefore finishes the thread run
    }
    ...
    public final void run() {
        ...
        while (!landoFinished){
            try{
                ...
                Callback callback = whichCallBack.get(message.getClass());
