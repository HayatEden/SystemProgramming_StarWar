...
public class MessageBusImpl implements MessageBus {
	...
	private ConcurrentHashMap<MicroService,LinkedBlockingQueue<Class<? extends Broadcast>>> subedBroadcastList; // keep track of what broadcasts m is subed to so we can delete him from it at the end
	private ConcurrentHashMap<MicroService,LinkedBlockingQueue<Class<? extends Event>>> subedEventList; // keep track of what broadcasts m is subed to so we can delete him from it at the end
	...
	private MessageBusImpl(){
		...
		subedBroadcastList = new ConcurrentHashMap<>();
		subedEventList = new ConcurrentHashMap<>();
		lock1 = new Object();
		lock2 = new Object();
		lock3 = new Object();
		lock4 = new Object();
	...
	public <T> void subscribeEvent(Class<? extends Event<T>> type, MicroService m) {
		synchronized (lock3) {
			...
			if (subedEventList.containsKey(type)){
				subedEventList.get(m).add(type);
			}
			else {
				LinkedBlockingQueue<Class<? extends Event>> toAdd = new LinkedBlockingQueue<>();
				toAdd.add(type);
				subedEventList.put(m,toAdd);
			}
		}
	}
	...
	public synchronized void subscribeBroadcast(Class<? extends Broadcast> type, MicroService m) {
		synchronized (lock4) {
			...
			if (subedBroadcastList.containsKey(type)){
				subedBroadcastList.get(m).add(type);
			}
			else {
				LinkedBlockingQueue<Class<? extends Broadcast>> toAdd = new LinkedBlockingQueue<>();
				toAdd.add(type);
				subedBroadcastList.put(m,toAdd);
			}
		}
	}
	...
	private void removeFromAll(MicroService m){ // deletes all m references
		if (subedEventList.containsKey(m)) {
			for (Class c : subedEventList.get(m)) {
				subToEvent.get(c).remove(m);
			}
		}
		if (subedBroadcastList.containsKey(m)) {
			for (Class c : subedBroadcastList.get(m)) {
